...
/* ------------------------------------------------------------
   3D SCENE — wrapped so a WebGL failure (disabled GPU, locked-down
   tablet browser, context loss) never blocks the rest of the page.
------------------------------------------------------------ */
...
function initScene() {
  ...
  const SHARD_COUNT = isSmallScreen ? 14 : 26;
  ...
  /* ---------- Particle Field ---------- */
  const PARTICLE_COUNT = isSmallScreen ? 700 : 1600;
  ...
  // If the GPU context is lost mid-session, stop cleanly instead of
  // spamming errors or leaving a frozen frame.
...
/* ------------------------------------------------------------
   ...
   WebGL failure never leaves the page stuck (e.g. loader visible
   forever, nav/menu unresponsive).
------------------------------------------------------------ */
...
window.addEventListener('load', () => setTimeout(finishLoading, 1400));
// Fallback in case the load event already fired, never fires, or the
// 3D setup above threw before this point was reached.
